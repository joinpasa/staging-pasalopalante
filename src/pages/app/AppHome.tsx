...
import { useAppBadges, useAppMe, useMovementTotals, useMyRecentActs } from "@/hooks/useAppData";
import { actEmoji, modeLabel, timeAgo } from "@/lib/appActs";
...
export default function AppHome() {
  ...
  useEffect(() => {
    ...
    if (!standalone) {
      navigate(`/app/join?ref=${encodeURIComponent(ref)}`, { replace: true });
    }
      ...
      {user && earned.length > 0 && (
        <section>
          <div className="mb-3 flex items-baseline justify-between">
            ...
            <Link to="/app/badges" className="text-sm font-semibold text-app-coral">
              See all
            </Link>
          </div>
      ...
      <section>
        ...
        {user && (myActs?.length ?? 0) > 0 ? (
          ...
          <div className="rounded-2xl bg-app-surface p-5 text-sm leading-relaxed text-muted-foreground">
            ...
            <div className="mt-3">
              <Link to={user ? "/app/log" : "/app/wall"} className="font-semibold text-app-coral">
