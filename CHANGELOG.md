...
- **Added two new "Get Involved" landing pages with embedded sign-up forms**: `/get-involved/schools` (Schools & Educators) and `/get-involved/nonprofits` (Nonprofits & Faith Communities), each embedding the GoHighLevel form for that audience. The "Bring it to your school" and "Partner with us" cards on `/get-involved` now link to these instead of the generic Contact page.
