...
- **The app now lives at `pasalopalante.com/app/*`, restoring true one-tap "Get the app → native install dialog."** Since a website can never trigger a *different* domain's install prompt, getting back the exact one-tap experience the old single-domain Lovable build had meant putting the marketing pages and the app back on one domain. `app.pasalopalante.com` still exists for anyone with an old bookmark or a previously-shared Pass QR code.
  ...
  - Also cut the visible flash between tapping "Get the app" and the native dialog appearing — previously our own "Install app" card and a moment of full app content both flashed by first; now it goes straight to the native dialog with the splash screen held until that's ready.
  - `app.pasalopalante.com`'s own Cloudflare project can be pointed at a redirect once this is confirmed stable — not done yet.
